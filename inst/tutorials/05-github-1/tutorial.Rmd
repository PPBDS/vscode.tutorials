...
<!-- DK: Prompts are wrong. -->

<!-- Can just assert that you should be in a repo, which you have done several times already. -->

<!-- Maybe just delete? Much of this is junk, like creating a qmd by hand, section by section. Much of this was covered in introduction and quarto. -->
...
You must do this tutorial in a Codespace created from a Github repo named '05-github-1'. If you haven't already done this, follow these steps:

1. Create a GitHub Repo: go to the [codespace-starter repo] (https://github.com/ppbds-student/codespace-starter). Click the green "Use this Template" drop-down button and select "Create a new repository". On the next page name the repo `05-github-1`. Then click "Create repository".

2. Create a Codespace from `05-github-1`: From this repo's page, click the green "Code" drop down button, in the top right, then select the "Codespaces" tab and then click "Create Codespace on main".
...
````
https://github.com/ppbds-student/05-github-1
...
````
@ppbds-student ➜ /workspaces/05-github-1 (main) $ pwd
/workspaces/05-github-1
@ppbds-student ➜ /workspaces/05-github-1 (main) $
````

You should be in a directory named `05-github-1`. By default, creating a repo from the Codespace starter and opening a Codespace creates this root directory.
...
````
@ppbds-student ➜ /workspaces/05-github-1 (main) $ ls
@ppbds-student ➜ /workspaces/05-github-1 (main) $ 
````
...
````
@ppbds-student ➜ /workspaces/05-github-1 (main) $ ls -a
.  ..  .devcontainer  .git
@ppbds-student ➜ /workspaces/05-github-1 (main) $
````
...
````
@ppbds-student ➜ /workspaces/05-github-1 (main) $ cat analysis-1.qmd 
---
title: "Analysis 1"
...
````
@ppbds-student ➜ /workspaces/05-github-1 (main) $ ls -R analysis-1_files
analysis-1_files:
libs
...
````
@ppbds-student ➜ /workspaces/05-github-1 (main) $ cat .gitignore
analysis-1_files
@ppbds-student ➜ /workspaces/05-github-1 (main) $ 
````
...
````
@ppbds-student ➜ /workspaces/05-github-1 (main) $ git log
...
If you look at the GitHub page for `05-github-1`, you will see that the changes have been pushed to GitHub. If your computer stops working, you won't lose any of your work.
...
Setting `execute: echo: false` in the YAML header applies to the whole document: every code chunk still runs, but only its output appears in the rendered HTML --- the code itself is hidden. That is usually what you want for a published page, where readers care about the results, not the code that produced them.
...
(In the **AI Introduction** tutorial you also met the Gemini CLI, an agent that can edit your files directly. Either approach is fine throughout these Git tutorials: ask an AI in a chat window and copy/paste the code by hand, or let the Gemini CLI write it into your file for you. Use whichever you prefer.)
...
````
@ppbds-student ➜ /workspaces/05-github-1 (main) $ git log -n 1
...
Let's publish our results to [GitHub Pages](https://pages.github.com/). Before we do, go to the GitHub repo for `05-github-1`, click on the "Settings" top menu item, and then click on the "Pages" side menu item.
...
````
@ppbds-student ➜ /workspaces/05-github-1 (main) $ quarto publish gh-pages analysis-1.qmd
? Publish site to https://ppbds-student.github.io/05-github-1/ using gh-pages? (Y/n) › Yes
````
...
remote:      https://github.com/ppbds-student/05-github-1/pull/new/gh-pages        
remote: 
To https://github.com/ppbds-student/05-github-1
...
From https://github.com/ppbds-student/05-github-1
...
Go to your `05-github-1` page on GitHub. Refresh the page. `quarto publish` has changed some of the items. Copy and paste all the text in the "Visit site" box.
...
Publishing to GitHub Pages has not changed any of the files in our repo, so the "Source Control" button does not show any changes for us to deal with. Our local repo matches our `05-github-1` on GitHub. We are done!

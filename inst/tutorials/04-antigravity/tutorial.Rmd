...
Notice that our document is titled "Exploring Storms" while we poke at `flights`. That is fine. A document's title describes its *deliverable*, not everything you try along the way --- these exploration chunks are temporary, and we will delete them before publishing.
...
```
Update the storms-per-year chart in analysis.qmd to be a stacked area chart showing the count of each storm type per year. Filter to just tropical depressions, tropical storms, and hurricanes. Use a color scale that goes from light blue for tropical depressions through orange for tropical storms to dark red for hurricanes. Also delete the two flights chunks --- that exploration is done. Re-render the file when done.
```

...
The [Antigravity CLI](https://antigravity.google/product/antigravity-cli) is a free, open AI assistant built by Google that runs in your terminal. Unlike a web chat interface, it can take actions on your behalf: reading files, running terminal commands, and writing new files. Each time it wants to take an action, it asks for your permission first, at least by default.
...
and hit `Enter`. New College of Florida is providing some funding for your use of Antigravity. But, after the course is over, you can get the same sort of access for $20 or so per month. Select "Continue with Google Cloud."
...
Doing will probably bring up this screen. Obviously, you should click "Open."

```{r}
include_graphics("images/agy-5.png")
```

The next step might ask you to confirm the your GitHub account. **You must use your NCF email account.** 

As Google changes things, this sequence of images won't match exactly what you see. Common sense should go you through it all. The most important point was the to choose 
...
```{r named-storms-per-year-5-test}
#| echo: true
flights |>
  filter(origin == "JFK") |>
...
```{r named-storms-per-year-6-test}
#| echo: true
flights_jfk <- flights |>
  filter(origin == "JFK") |>
  select(dep_delay, arr_delay, carrier)
...
```{r named-storms-per-year-7-test}
#| echo: true
flights_jfk <- flights |>
  filter(origin == "JFK") |>
  select(dep_delay, arr_delay, carrier)
...
```{r named-storms-per-year-8-test}
#| echo: true

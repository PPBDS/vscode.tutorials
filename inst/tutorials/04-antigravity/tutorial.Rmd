...
You will then be asked to select a region. Select `us` and hit `Enter`. 
...
Same principle as in the Quarto tutorial: if a file can be *regenerated* from your source, it doesn't belong in the commit history. The agent may write a more thorough `.gitignore` than your hand-made one --- extra entries, or wildcard patterns like `*_files`, are harmless. Delegating housekeeping files like this is the normal workflow from now on.
...
Based on the current state of the repository, the project is in its early stages. Here is a summary of what has been done:
   ...
   1. Initial Setup: The project contains an AGENTS.md instructions file and a .gitignore file.
  ...
  There are no memory records from previous sessions. Would you like to continue the analysis or add new features to this report?
````
...
Ask Antigravity to consolidate `analysis.qmd` so it contains only the setup chunk and the named-storms-per-year line chart. Delete any other chunks. Render. In the bash Terminal, run `cat analysis.qmd`. CP/CR.

Throughout this tutorial we will often say "ask Antigravity to" when we want you to use AI.

```{r antigravity-in-tutorials-2}
question_text(NULL,
	answer(NULL, correct = TRUE),
	allow_retry = TRUE,
	try_again_button = "Edit Answer",
...
Ask Antigravity to add a **new chunk** at the bottom of `analysis.qmd` that prints the `storms` dataset. Render. In the R Terminal, run `show_file("analysis.qmd", chunk = "Last")`. CP/CR.

```{r the-working-code-chunk-1}
question_text(NULL,
	answer(NULL, correct = TRUE),
	allow_retry = TRUE,
	try_again_button = "Edit Answer",
...
```{r the-working-code-chunk-1-test}
#| echo: true
storms
```
...
Printing a raw tibble like this would never survive into a published document. That is fine --- the working chunk's output is for *you*, not your readers, and we will clean it up before publishing.

### Exercise 2

Ask Antigravity to update the last chunk to pipe `storms` through `glimpse()`. Render and inspect the output. In the R Terminal, run `show_file("analysis.qmd", chunk = "Last")`. CP/CR.

```{r the-working-code-chunk-2}
question_text(NULL,
	answer(NULL, correct = TRUE),
	allow_retry = TRUE,
	try_again_button = "Edit Answer",
...
```{r the-working-code-chunk-2-test}
#| echo: true
storms |>
  glimpse()
```
...
Ask Antigravity to update the working chunk to count the observations in `storms` by `status`. Render. In the R Terminal, run `show_file("analysis.qmd", chunk = "Last")`. CP/CR.

```{r the-working-code-chunk-3}
question_text(NULL,
	answer(NULL, correct = TRUE),
	allow_retry = TRUE,
	try_again_button = "Edit Answer",
...
```{r the-working-code-chunk-3-test}
#| echo: true
storms |>
  count(status)
```
...
Ask Antigravity to update the working chunk to filter `storms` to observations with `year >= 2000` and select the `year`, `name`, `status`, `wind`, and `pressure` columns. Leave the result as the final expression so it prints. Render. In the R Terminal, run `show_file("analysis.qmd", chunk = "Last")`. CP/CR.

```{r the-working-code-chunk-4}
question_text(NULL,
	answer(NULL, correct = TRUE),
	allow_retry = TRUE,
	try_again_button = "Edit Answer",
...
```{r the-working-code-chunk-4-test}
#| echo: true
storms |>
  filter(year >= 2000) |>
...
Ask Antigravity to save the filtered tibble to a variable called `storms_recent` instead of printing it directly. Render and notice the table disappears from the HTML. In the R Terminal, run `show_file("analysis.qmd", chunk = "Last")`. CP/CR.
...
Add `#| cache: true` to the `storms_recent` chunk. Render. In the bash Terminal, run `ls`. CP/CR.
...
Caching is a render-time feature. The first render writes the chunk's result to disk. Later renders skip re-running the chunk and load from disk instead. `analysis_cache/` appearing in `ls` confirms it is wired up. Data preparation is the natural chunk to cache: in real projects it is often the slow part --- downloading files, joining tables, fitting models --- and its result rarely changes. Filtering `storms` is fast, but the habit scales.

### Exercise 3

Ask Antigravity to add `analysis_cache` to `.gitignore`. Note that the Source Control badge count in VS Code has increased because the cache directory is now visible to git --- adding `analysis_cache` to `.gitignore` makes it drop back. In the bash Terminal, run `cat .gitignore`. CP/CR.
...
Ask Antigravity to add a new code chunk with a **ggplot2** scatter plot of `pressure` vs. `wind`, using `storms_recent`. Render. In the R Terminal, run `show_file("analysis.qmd", chunk = "Last")`. CP/CR.
...
```{r plotting-1-test}
#| echo: true
ggplot(storms_recent, aes(x = wind, y = pressure)) +
  geom_point()
```
...
The first version of a plot is rarely the last. Ask Antigravity to improve the working chunk: color the points by `status`, make them semi-transparent so overlapping observations show, and add a meaningful title, subtitle, and axis labels. Render and check the Live Server tab. In the R Terminal, run `show_file("analysis.qmd", chunk = "Last")`. CP/CR.

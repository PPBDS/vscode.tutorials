...
Your output shows the full `devcontainer.json` from the `codespace-starter` repo. You will explore each part of this file in a later tutorial, `11-codebase-start`. Keep this Codespace open --- you will need it alongside a second Codespace in the next section.
...
2. Use one of the AI command-line assistants (`gemini`, `claude`, `codex`, `agy`, or `aider`) in a bash Terminal in your `codespace-starter` Codespace.
...
This is a completely different way of installing an extension than the `customizations.vscode.extensions` array you just found in `devcontainer.json`. Installing through the Marketplace UI only affects *this* running Codespace --- it is not saved anywhere, so the next time you rebuild or open a fresh Codespace from this repo, Rainbow CSV will be gone. The JSON array, by contrast, is declarative and committed to the repo: it installs automatically for anyone --- including your future self --- who opens a Codespace from `r-devcontainer`, with no manual step required.

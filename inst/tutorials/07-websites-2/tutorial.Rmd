...
Create a public GitHub repo (called `website-1`). Set the "Add README" toggle switch to "On." Copy/paste the URL for its GitHub location.
...
Create a public GitHub repo called `website-2`. Set the "Add README" toggle switch to "On."
...
Create a public GitHub repo called `website-3`. Set the "Add README" toggle switch to "On."
...
"Source" the entire script by choosing the "Source R File with Echo" menu option from the "Source" button the upper right (the right-pointing right triangle). This takes all the code from the script and feeds it, line-by-line, to the R Terminal. CP/CR.
...
Restart your R session with `Cmd/Ctrl + Shift + 0`. When R restarts, you should still be in the `website-3` project. At the R Terminal, run `library(tidyverse)`. We need to reload the **tidyverse** every time we restart R.

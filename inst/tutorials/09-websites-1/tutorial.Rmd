...
In later projects, you will often just ask AI to create a website's files and put the site on the web. That is OK! But, at least for now, issue the `quarto publish gh-pages` command yourself, at the Terminal. It is guaranteed to handle the subtle Git branching issues, and their interaction with GitHub Pages, that AI sometimes messes up. By the end of this section you will know exactly what the command does behind the scenes --- which is how you audit anyone, human or AI, who publishes on your behalf.
...
Publishing left your source untouched --- but it did add something to the repo. From the Terminal, run `git branch -a`. CP/CR.
...
What is on that other branch? From the Terminal, run `git ls-tree --name-only gh-pages`. CP/CR.

...
Most questions ask for **CP/CR** --- **C**opy/**P**aste the **C**ommand and **R**esponse --- as you learned in the Tutorials tutorial.
...
CP/CR the useful part of the script's response. If the window reload cleared the output, paste the first prompt you see in the new `workflow` Terminal instead.
...
Your output may say that the repository was created, cloned, or was already present. All three are fine. The result is a GitHub repository named `workflow` and a folder at `/workspaces/workflow`.
...
Stop and fix the open folder if your result still ends in `codespace-starter`. All remaining work belongs in `/workspaces/workflow`.
...
You met **GitHub Copilot** --- the AI assistant built right into VS Code --- in Workspace, where it wrote small files for you. Now give it real work. We use Copilot here because it is always available and works out of the box.

Open a new R Terminal. Keep the **R Tutorial** terminal running, and use this new R Terminal for the R commands below. Click the small arrow beside the `+` in the top-right corner of the Panel, then choose **R Terminal**. (The new terminal's tab is labeled **R Interactive**.) The red boxes show where to click.

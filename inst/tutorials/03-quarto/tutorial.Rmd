...
In the bash Terminal, run `rm -rf quarto-*` to delete the practice files and directories in your `quarto` repo. Then run `ls` to confirm that only `AGENTS.md` remains. CP/CR.
...
`rm` removes files; the `-r` option lets it descend into directories, which is needed for the `_files` directories that rendering created; `-f` skips any are-you-sure prompts. The `quarto-*` pattern is a **wildcard**: it matches everything whose name begins with `quarto-`, so one short command removes all the practice files at once --- while leaving `AGENTS.md`, and the hidden `.git` directory that is the repo itself, untouched. Be careful with `rm`: unlike moving files to the Trash, it is (usually) irreversible. Wildcards get a full treatment in a later Terminal tutorial; for now, notice how much one small pattern accomplishes.

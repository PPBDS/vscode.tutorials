...
<!-- DK: Need to add quarto publish gh-pages analysis.qmd.
...
You should be doing this tutorial in a repo called `quarto`. Recall from the Introduction tutorial: a repo is a folder that Git watches, with a copy kept on GitHub. If your Explorer already shows a `quarto` folder, you are all set. If it shows something else --- `codespace-starter`, or the `code` repo from the last tutorial --- the next two exercises will get you connected. Either way, they only take a minute.
...
If your result ends in `quarto`, you are all set --- **do not run the script in the next exercise**; just do its last step. Otherwise, the next exercise connects you.

### Exercise 2

**Skip this paragraph --- do not run the script --- if `pwd` showed `/workspaces/quarto`.** Otherwise, run:
...
Restart your R session by opening a new R Terminal and closing your previous one. As always, you are doing this work in an R Terminal different from the one that is running this tutorial. If you mistakenly restart R in this terminal, this tutorial will quit. That is OK! You can restart it the same way you started it: click **R Tutorials** in the Activity Bar, find this tutorial under **vscode.tutorials**, and click the arrow to launch it. All of your saved answers will still be there.
...
Before moving on, delete the practice files you created in this tutorial. They were exercises, not work worth keeping.

...
If your result ends in `quarto`, you are all set. **Do not run the connect-repo commands in the next exercise**. Otherwise, the next exercise connects you.
...
The `connect-repo.sh` script lives in `codespace-starter`, so the first command moves you there; running the script from anywhere else --- including the `code` repo from the last tutorial --- fails. If a sign-in message appears, follow it: open the link in your browser, authorize GitHub, then return to VS Code. The script may reload the VS Code window or ask you to open a folder. If the Explorer does not show `quarto`, use `File -> Open Folder...` and open `/workspaces/quarto`. 

###

The reload will often start a new copy of the tutorial, with your answers already filled in. You know have two tabs open to the same tutorial. Don't fret! These are just two windows open to the same object. You can close one of them. Doesn't matter which.The other will continue to work.

The reload might also just stop this tutorial. That is OK! Restart it the same way you started it: click **R Tutorials** in the Activity Bar, find this tutorial under **vscode.tutorials**, and click the arrow to launch it. 

All of your saved answers will still be there.
...
```{r}
include_graphics("images/q-1.png")
```
...
```{r}
include_graphics("images/q-2.png")
```
...
```{r}
include_graphics("images/q-3.png")
```

###

If you did save the file, no worries! **It is never a problem if your answer to a tutorial exercise differs from ours.** What matters is that you made an honest effort to follow the instructions. 

###

Quarto is a command line interface tool, not an R package. Therefore, its commands are largely run in a bash Terminal. As you work through this tutorial, and use Quarto in the future, you should refer to the [Quarto documentation](https://quarto.org/). Of course, in the modern world, we ask an AI when we have a problem.
...
```{r}
include_graphics("images/q-4.png")
```

Now that the file is saved, the black dot has disappeared. The file is now part of the repo, and will be tracked by Git. The `U` next to the name means that, according to Git, the file is **untracked**. Git does not yet recognize it, because we have not added it to the repo. 
...
```{r}
include_graphics("images/q-5.png")
```

This command splits the Editor and opens a new tab titled "Quarto Preview" on the right. This displays the rendered document. It's often better to preview the document in a separate browser window. VS Code should have offered you the opportunity to open the HTML file in a browser. Ignore that for now.

###

There is a lot going on, so we will consider different parts of the window. Please pay attention.

```{r}
include_graphics("images/q-5-a.png")
```

The Quarto Preview window shows what your Quarto document looks like now that it has been rendered as HTML. The first red arrow indicates the "refresh" button, which re-renders the document, although this refresh generally happens automatically. 

The second red arrow indicates the URL at which the rendered page is actually available (at least to you), via your Codespace.

The third red arrow indicates the "Open in Browser" button, which opens the rendered document, using that same URL, in a browser window. 

###

The list of terminals on the righthand side of the window shows four entries. The first is the bash Terminal which initialized the Codespace. The second is the bash Terminal associated with running `connect-repo`. The third is the bash Terminal that we opened to run `pwd` and `ls`. 

```{r}
include_graphics("images/q-5-b.png")
```

The fourth is the bash Terminal associated with the Quarto Preview. This last terminal is where the `quarto preview` command is running, which is what generates the rendered document.

###

Using the `Cmd/Ctrl + Shift + K` shortcut, causes VS Code to open a new bash Terminal and issue the `quarto render` command, along with some options, as you can see below.

```{r}
include_graphics("images/q-5-c.png")
```

You don't need to worry about this now. Just use `Cmd/Ctrl + Shift + K` to preview your Quarto documents. Remember: Professionals don't click buttons. They save time by using keyboard shortcuts.

###

Lastly, note that you also open the HTML file in a browser window by using `Cmd/Ctrl + click` on the "Browse at" link. 

```{r}
include_graphics("images/q-5-d.png")
```
...
`.gitignore` tells Git which files to leave alone. The `*` makes the rule cover the support directory of *any* rendered document, including ones you have not created yet. The rendered HTML itself stays tracked --- unlike the `_files` directory, it is the artifact your readers will eventually see.
...
Your first Quarto document is done, so commit it. Through the Source Control view, as you did in the Code tutorial: review **Changes** --- you should see `.gitignore`, `quarto-1.qmd`, and `quarto-1.html`, but no `_files` directory, thanks to your `.gitignore` --- type `Add quarto-1` as the message, press **Commit**, then **Sync Changes**.
...
`main` and `origin/main` on the same line: your Codespace and GitHub agree. One document finished, one commit made. You will repeat this rhythm at the end of the next section.
...
Preview the file. You should see your plot --- but not the code that made it. The `execute: echo: false` you added to the YAML header in Exercise 7 applies to every chunk in the document, including this new one. That is exactly what we want: a reader sees the graphic, not the machinery.
...
That instruction --- "commit and push," and nothing more --- is all later tutorials will give you, and now it is all you need. `git status` is the quick way to confirm nothing was left behind: a clean working tree, up to date with `origin/main`, means every piece of your work is safely on GitHub.
...
Notice the shape your document has grown into: a YAML header, a `setup` chunk that loads libraries, then a work chunk that makes a graphic. That is the skeleton of every analysis document you will make from now on.
...
Published to https://your-username.github.io/quarto/quarto-2.html
quarto $
````

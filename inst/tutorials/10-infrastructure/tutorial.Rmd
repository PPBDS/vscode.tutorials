...
As with your earlier tutorials, you do this one in your own repo named `10-infrastructure`. To set that up:

1. Launch a Codespace: go to the [codespace-starter repo](https://github.com/PPBDS/codespace-starter). Click the green "Code" drop-down button, select the "Codespaces" tab, and click "Create codespace on main".

2. Create your work repo: in the bash Terminal, run

````
.devcontainer/connect-repo.sh 10-infrastructure
````

The first time, a browser prompt asks you to **authorize** --- click through it, then come back. The script signs you in as yourself and creates a new public `10-infrastructure` repo under your GitHub account.

3. Open your repo as the workspace: **File → Open Folder → `/workspaces/10-infrastructure`**.

Copy and paste the URL for your `10-infrastructure` repository.
...
````
https://github.com/ppbds-student/10-infrastructure
````
...
````
10-infrastructure $ pwd
/workspaces/10-infrastructure
10-infrastructure $ ls -a
.  ..  .git
10-infrastructure $
````
...
````
10-infrastructure $ cd ../codespace-starter
...
Both repos live side by side inside this Codespace, under `/workspaces`: your own `10-infrastructure` and the `codespace-starter` you launched from. The `..` in `cd ../codespace-starter` means "the parent directory," so the command moves up from `/workspaces/10-infrastructure` to `/workspaces`, then down into `codespace-starter`. That is where the `devcontainer.json` that configured this machine actually lives --- not in your work repo, which is just for saving your own work. Notice the prompt change from `10-infrastructure $` to `codespace-starter $`: the prompt always shows the name of the directory you are in, a quick way to confirm where you are.
...
Your output shows the full `devcontainer.json` from the `codespace-starter` repo. You will explore each part of this file in the Codespace Starter Devcontainer section. Keep this Codespace open --- you will need it alongside a second Codespace in the next section.
...
1. Copy and paste the entire document to an AI chatbot --- ChatGPT, Claude, Gemini, or any other.

2. Use one of the AI command-line assistants (`claude`, `codex`, `agy`, or `aider`) in a bash Terminal in your `codespace-starter` Codespace.
...
```
tutorial.helpers::show_file(".devcontainer/devcontainer.json")
```
...
```
"image": "ghcr.io/ppbds/devcontainers/student:0.6.0",
```
...
`codespace-starter` uses a custom PPBDS image **locked** to version `0.6.0`, so every student gets the exact same setup regardless of when they open the Codespace. It pre-installs everything the course needs: Quarto, `gh`, the `arf` R console, `httpgd`, `pak`, the `tutorial.helpers` and `vscode.tutorials` packages, a Python data-science stack, and four AI command-line assistants --- `claude`, `codex`, `agy`, and `aider`.
...
Like the Jupyter template's `updateContentCommand`, this runs automatically when the Codespace is created. It refreshes the two fast-moving course packages --- `tutorial.helpers` and `vscode.tutorials` --- to their latest versions, so you always get the newest tutorials even though the image itself is pinned. The image bakes in working copies as a fallback, so a momentary GitHub outage never blocks the Codespace from opening.

...
description: A tour of the working environment - the bash Terminal, the R Terminal, and GitHub Copilot, ending with the ways to run R code.
---
...
This tutorial, intended for beginners, tours the working environment you will use for everything that follows: the bash Terminal, the R Terminal, and [GitHub Copilot](https://github.com/copilot), an AI coding assistant, ending with the different ways to run R code. Complete it inside a [GitHub Codespace](https://github.com/features/codespaces), as set up [here](https://ppbds.github.io/primer/getting-started.html). You should already have completed the **Tutorials** tutorial from the [**tutorial.helpers**](https://ppbds.github.io/tutorial.helpers/) package.
...
```{r}
include_graphics("images/gs-1.png")
```
...
```{r}
include_graphics("images/gs-2.png")
```
...
The other Mac/Windows issue to be aware of is the Command key (Mac) versus Control key (Windows). 

Many shortcut key commands use the Command key --- abbreviated to `Cmd` --- or the Control key --- abbreviated to `Ctrl` --- to perform various actions, depending on the operating system you are using. We will use `Cmd/Ctrl` to mean either key, as appropriate.
...
`pwd` stands for **p**rint **w**orking **d**irectory --- the folder the shell is currently "in." GitHub Codespaces place your repository under `/workspaces/`, so that is where you begin. Almost every command you run acts on this directory unless you tell it otherwise.
...
`head` prints the first ten lines of a file, a quick way to peek at its contents without opening it. Because `codespace-starter` is not your own repo, you will soon learn to create your own with `.devcontainer/connect-repo.sh` and save your work there.
...
`Ctrl + C` interrupts whatever process is currently running in the terminal and returns you to the prompt; the `^C` marks where you pressed it. When a command hangs or you change your mind, `Ctrl + C` stops it. You will use it again in the next exercise to stop the tutorial itself.
...
* Return to your Codespace and click the terminal tab titled **R Tutorial** on the right side of the Panel.

```{r}
include_graphics("images/gs-3.png")
```

* Stop the tutorial by pressing `Ctrl + C` in that terminal. This is useful because it can interrupt any terminal process. A new line should appear where you can enter another command.

* To restart the tutorial, press the triangle button next to the "Getting Start" tutorial via the R Tutorials VS Code extension.
...
```{r}
include_graphics("images/gs-4.png")
```
...
```{r}
include_graphics("images/gs-5.png")
```
...
Copilot might just create the file in your working directory without checking with you. 
...
```{r}
include_graphics("images/gs-6.png")
```
...
```{r}
include_graphics("images/gs-6-b.png")
```

...
You should be doing this tutorial in a repo named `websites-1`. If you are not, create one and then connect to it. You may need to restart this tutorial after you do so.
...
Copy/paste the URL for your `websites-1` repository on GitHub.
...
```
https://github.com/ppbds-student/websites-1
```
...
In the bash Terminal, run `quarto create project website . --no-open`. Don't forget the `.` — it tells Quarto to place the new files in the current directory.
...
````
websites-1 $ quarto create project website . --no-open
? Title (website) › A New Website
Creating project at /workspaces/websites-1:
  - Created _quarto.yml
  - Created .gitignore
  - Created index.qmd
  - Created about.qmd
...
Why `--no-open`? By default, when you run `quarto create` from inside a VS Code terminal, Quarto finishes by asking VS Code to open the new project. In a Codespace, "open the project" means "open a new browser tab" — pointed at the very same Codespace and folder you are already looking at. Nothing is broken, but you end up with two tabs of the same workspace, which is confusing. The `--no-open` flag tells Quarto to create the files and stop. We may have set up your Codespace to add this flag automatically even when you forget it, but type it anyway: you should know what your commands do.
...
Commit and push all files with the message "initial version".

Run `git log -n 1` in the bash Terminal. CP/CR.
...
From the bash Terminal, run `quarto render`. CP/CR.
...
From the bash Terminal, run `ls _site`. CP/CR.
...
From the bash Terminal, run `quarto preview`. CP/CR.
...
In Codespaces, a pop-up notification should appear automatically offering to open the site in your browser. If it does not, click the URL shown in the bash Terminal's output.
...
Go to the bash Terminal. Copy and paste all the output from `quarto preview`.
...
At the bash Terminal, you should see a new message. Copy and paste it here.
...
To stop `quarto preview`, press `Ctrl + c` in the bash Terminal. This cancels any running terminal command.
...
In later projects, you will often just ask AI to create a website's files and put the site on the web. That is OK! But, at least for now, issue the `quarto publish gh-pages` command yourself, at the bash Terminal. It is guaranteed to handle the subtle Git branching issues, and their interaction with GitHub Pages, that AI sometimes messes up. By the end of this section you will know exactly what the command does behind the scenes --- which is how you audit anyone, human or AI, who publishes on your behalf.


### Exercise 1

Go to your `websites-1` Github repository.
...
````
https://github.com/ppbds-student/websites-1/settings/pages
````
...
At the bash Terminal, run `quarto publish gh-pages`. 
...
````
websites-1 $ quarto publish gh-pages
? Publish site to https://ppbds-student.github.io/websites-1/ using gh-pages? (Y/n) › Yes
...
remote:      https://github.com/ppbds-student/websites-1/pull/new/gh-pages
remote:
To https://github.com/ppbds-student/websites-1.git
...
Copy/paste the last 8 or so lines of the output from running `quarto publish gh-pages`. CP/CR.
...
[✓] Published to https://ppbds-student.github.io/websites-1/
...
````
https://ppbds-student.github.io/websites-1/
````
...
Publishing left your source untouched --- but it did add something to the repo. From the bash Terminal, run `git branch -a`. CP/CR.
...
`quarto publish` created a second branch, `gh-pages`, and pushed it to GitHub (that is the `remotes/origin/gh-pages` line). The `*` marks the branch you are on: `main`, exactly where you have been all along. Your repo now carries two parallel histories.

### Exercise 7

What is on that other branch? From the bash Terminal, run `git ls-tree --name-only gh-pages`. CP/CR.
...
````
websites-1 $ git ls-tree --name-only gh-pages
.nojekyll
about.html
index.html
search.json
...
No QMD files at all. The `gh-pages` branch holds the *rendered site* --- the same files you saw in `_site`. Behind the scenes, `quarto publish gh-pages` does four things: it renders the site into a temporary copy of the repo (a "worktree"), commits the resulting HTML to the `gh-pages` branch, pushes that branch to GitHub, and tells GitHub Pages to serve it. `main` keeps the source; `gh-pages` keeps the product. That division is why the Source Control pane shows no changes after publishing --- nothing on `main` moved. (The extra `.nojekyll` file tells GitHub Pages to serve the files exactly as they are.)

### Exercise 8

Earlier, you visited your repo's Pages settings *before* publishing. Go back: from your repo's page on GitHub, click **Settings**, then **Pages**. Copy/paste the sentence at the top which describes your site.
...
````
Your site is live at https://ppbds-student.github.io/websites-1/
````

###

The publish command changed these settings for you: the deployment source is now the `gh-pages` branch. When a publish goes wrong --- yours or an AI agent's --- this page is where you check what GitHub Pages is actually serving.
...
From the bash Terminal, run `ls .quarto`. CP/CR.
...
From the bash Terminal, run `quarto preview`. Copy and paste the text from the home page once it comes up on your browser.
...
From the bash Terminal, stop the `quarto preview` command with `Ctrl + c`. Run `quarto publish`. You may need to hit `Enter` to confirm that you still want to use GitHub Pages. Refresh once after you are on your website.
...
````
websites-1 $ quarto publish
? Publish update to: › https://ppbds-student.github.io/websites-1/ (GitHub Pages)
From https://github.com/ppbds-student/websites-1
...
origin  https://github.com/ppbds-student/websites-1 (fetch)
origin  https://github.com/ppbds-student/websites-1 (push)
To https://github.com/ppbds-student/websites-1
...
Commit and push all the files in the project. Run `git log -n 2` in the bash Terminal. CP/CR. 

...
Quarto is a command line interface tool, not an R package. This means that help is, by-and-large, not available through `?` at the R Terminal. Instead, as you work through this tutorial, and use Quarto in the future, you should refer to the [Quarto documentation](https://quarto.org/). Of course, in the modern world, we just ask an AI when we have a problem.
...
Restart your R session. The best way to do this is with the shortcut key `Cmd/Ctrl + Shift + 0`. There is also a "Restart console" button in the upper right portion of the R Terminal tab. 

```{r}
include_graphics("images/add-numbers.png")
```

Restarting the R Terminal just means restarting R.
...
**Important Point**: There are two different "worlds" with which we are dealing. First is the world of the Quarto document, the QMD file. Second is the world of the R Terminal in our current Positron window. **These worlds only connect when we connect them**. Something written in a Quarto document --- or an R script --- is not, by default, part of our current R session.
...
* `Cmd/Ctrl + Enter` will run all the code in the code chunk.
...
Restart your R session with `Cmd/Ctrl + Shift + 0`. As always, you are doing this work in your *other* Positron window, the one in which this tutorial is *not* running. If you mistakenly restart R in this window, this tutorial will quit. Which is OK! You can just restart it with `learnr::run_tutorial(name = "02-quarto", package = "vscode.tutorials")`.
...
**Notice the sloppy language!** There are two ways in which we can "load" **tidyverse** corresponding to the two worlds we are working in simultaneous: QMD World and R Terminal World.

...
Open a new R Terminal. This new terminal is the location in which you will do all the work for the tutorial. The current R terminal is just used to run this tutorial. 


## Terminal Overview
### 

We use the bash shell to communicate with the codespace computer. Bash is an example of a UNIX shell. There are others, including zsh, but they all behave very similarly. Regardless of the specific program, we often use the word "shell" or "terminal" to refer to them.

This section will help introduce you to the shell if you have not worked at the command line before, or it will serves as a refresher if you have.

### Exercise 1

Start by clicking on the bash terminal. This brings the shell to the foreground.
...
```         
@davidkane9 ➜ /workspaces/github-1 (main) $ 
```

Prompts differ. They often include the username, which is `davidkane9` for me. They also usually specify the current directory, which is `github-1` in this case because we this work by creating a new repo: `github-1`. Prompts usually end with a character like `$` or `%`. 
...
```         
@davidkane9 ➜ /workspaces/github-1 (main) $ pwd
/workspaces/github-1
@davidkane9 ➜ /workspaces/github-1 (main) $ 
```
...
Going forward, make sure to pay special attention to whether given commands are supposed to be run in the *bash terminal* or the *R Terminal*.

<!-- DK: Capitalization above? -->
...
Either way, R should start in the R Terminal. Copy/paste everything in the R Terminal from "Type 'demo()'" through the prompt.
...
Confirm that your setup is correct. In the Terminal (not the R Terminal), run `git remote -v`. CP/CR.
...
Create a PAT by using the `usethis::create_github_token()` function in the R Terminal. This should redirect you to a GitHub page about creating a PAT.
...
Run `gitcreds::gitcreds_set()` in the R Terminal and follow the prompts, providing your token when asked.

### 

Restart your R session with `Cmd/Ctrl + Shift + 0`. This is **not** the same thing as restarting Positron.

Run `usethis::git_sitrep()` in the R Terminal. CP/CR.

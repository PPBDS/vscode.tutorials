...
```         
project-1 $ quarto preview /workspaces/project-1/quarto-1.qmd --no-browser --no-watch-inputs
pandoc 
  to: html
...
Select the Explorer icon in the Activity Bar. Then, click the new document button to the right of the "PROJECT-1" heading. 
...
Use AI to create a beautiful graphic using the `penguins` data. Phrase the question however you like. Either AI approach is fine: ask a chat window (like ChatGPT or Grok) and copy/paste the code by hand, or let the Gemini CLI agent from the **AI Introduction** tutorial write it into your file directly. I asked [chatGPT](https://chatgpt.com/):
...
````
project-1 $ quarto publish gh-pages quarto-1.qmd
...
Commit and push `quarto-1.qmd` and `quarto-1.html` with the message "add AI graphic". Then from the Terminal, run `git log --oneline`. CP/CR.
...
<!-- DK: Question from Krrish: Also, I just had a question on one of the exercises. It asks me to delete the codespace because all of the code is saved because of the commit button. However, if I delete the codespace, the R tutorial will be gone and that means I will lose progress of my vscode.tutorial and if I create a new codespace, I will have to start all over again. I'm lost on what I should do? -->

Click the `...` next to your Codespace where you were working on the website and select "Delete".

Once it is deleted, go to the GitHub repo page for your website and look at the file list under the `<> Code` tab. You can see that all of the files you had created are there. In the upper right of this table, you can see that there have been 3 commits to the repo and that the last commit was made recently. Click on the "3 commits" button.
...
Commit and push `.gitignore`, `quarto-2.qmd`, and `quarto-2.html`. Use a sensible commit message.
...
Loading the **tidyverse** in the setup chunk makes its packages --- including **ggplot2** and the `mpg` dataset --- available to every other chunk in the document. Keeping all your `library()` calls in one chunk at the top is the standard way to organize a Quarto file.
...
Before committing, run `git status` from the Terminal to see what has changed. CP/CR. Then commit and push `quarto-2.qmd` and `quarto-2.html` with the message "load tidyverse, suppress echo".
...
Commit and push `quarto-2.qmd` and `quarto-2.html` with the message "add AI graphic". Then from the Terminal, run `git log --oneline`. CP/CR.

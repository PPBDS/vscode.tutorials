...
* moved around VS Code and used its Terminal;
* created your own GitHub repository;
* created and run R scripts by hand and with AI;
* made two plots using the `diamonds` data;
* committed your files with Git and pushed them to GitHub.
...
The first supplied the Codespace and the tutorial. The second is yours and will hold your work. `warmup.R` stays behind in the first folder, which is why it does not appear in your new Explorer.
...
The repository is almost empty on purpose. The hidden `.git` directory is enough to make it a Git repository. You will fill it with your own work as you go.

### Exercise 4

Now go see your repository on GitHub with your own eyes and make sure it is really there.
...
Authentication screens and model names change often, so your output may look different. A Google sign-in grants a temporary token; it does not give the program your Google password.

From this point forward, this course is **AI agnostic**. "Ask AI" means use whichever AI you prefer — Copilot, Antigravity, or any other agent the Codespace provides. We will generally show prompts and results, not tool-specific buttons or screenshots. The same rule as before still holds: stay in a review mode and never turn on full access.

...
Create a Github repo called `r4ds-2`. Set the "Add README" toggle switch to "On."
...
* `position = "identity"` places each object exactly where it falls, overlapping bars.
...
### Exercise 9
Ask AI to delete the `facet_wrap()` and continue the pipe by adding `coord_flip()` and use `show.legend = FALSE` so that there is no legend. Add this code as a continuation of your pipe to the same chunk in your QMD. Place your cursor at the beginning of the line where it says `cheese_tibble|> ` and run `Cmd/Ctrl + Enter`.
...
Ask AI how to read a `.xlsx` spreadsheet called `data/wine.xlsx` using `read_excel()`. Add this code to a new code chunk in your QMD. Place your cursor at the beginning of the line and run `Cmd/Ctrl + Enter`. Type `wine` in the R Terminal to retrieve the contents of `wine`.
...
###
Beyond `geom_text()` and `geom_label()`, **ggplot2** offers many annotation geoms like `geom_hline()` and `geom_vline()` for reference lines. These are often made thick and white (`linewidth = 2`, `color = "white"`) and drawn underneath the primary data layer to remain visible without distracting from the data.

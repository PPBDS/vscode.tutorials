...
In a previous tutorial, you found the `"image"` line inside `codespace-starter`'s `devcontainer.json` --- `ghcr.io/ppbds/devcontainer`, pinned to an exact version. That image does not appear out of nowhere --- it is built from a Dockerfile maintained by PPBDS. This tutorial reads through that real, current Dockerfile, section by section, to understand what it does and why it is written the way it is.

###

This is a **reading tour, not a building exercise**. You will not build this image yourself --- it is not buildable inside the course Codespace, and doing so is not the point. Instead, each exercise shows you a piece of the Dockerfile and asks you to find and quote a specific line or block from it, then explains what that piece does and why it exists. (You *will* write one tiny Dockerfile of your own along the way, just to feel the shape of one --- and then read the workflow that turns Dockerfiles like it into published images.)

###

You should be doing this tutorial in a repo named `the-devcontainer-dockerfile`. If you are not, create one and then connect to it. You may need to restart this tutorial after you do so.
...
This image is pinned by **digest** (the long `sha256:...` hash), not just by the `:4.6` tag. A tag like `:4.6` floats: Rocker can push a new build under that same tag any time, silently changing what future builds pull in. The digest locks the base to one exact, unchanging build. Bumping it is deliberate --- pull `:4.6` fresh, copy in the new digest, and commit the change on purpose, instead of being surprised by an upstream update. And because the digest quietly encodes the R version too, an R-version smoke test near the bottom of the file fails the build if a digest bump ever changes the R minor version unnoticed.

### Exercise 2

Time to build something of your own. In the bash Terminal, create a new file called `Dockerfile` (no extension) at the top level of your `the-devcontainer-dockerfile` repo, and write a simple Dockerfile that starts from a small base image and runs one command. For example:
...
A Dockerfile is just a short recipe: a `FROM` line picks a starting image (here, `alpine:3.20`, a tiny ~5 MB Linux distribution often used as a lightweight base), and each instruction after it adds one more layer on top. `CMD` sets the command that runs when a container starts from the image --- in this example, printing a greeting. It's the same shape as the much longer devcontainer Dockerfile you just read, just doing more work: more `FROM`, `RUN`, `COPY`, and `ENV` lines. You will not build this one yourself --- nobody at PPBDS builds the big one by hand either. The next two exercises show who does.

###

Your Dockerfile is a recipe; something still has to cook it. For `ghcr.io/ppbds/devcontainer`, that something is not a person typing commands --- it is a **GitHub Actions workflow** that lives in the same repo as the Dockerfile. Here is the part of it that matters, from `.github/workflows/build.yml`:
...
Find the step named `Build and push`. Copy and paste its `with:` block --- from `with:` through the two `cache-` lines.

```{r the-devcontainer-image-3}
question_text(NULL,
	answer(NULL, correct = TRUE),
	allow_retry = TRUE,
	try_again_button = "Edit Answer",
...
That one step is two `docker` commands in a trench coat. `docker build -t NAME .` reads the Dockerfile in the current directory (`context: .`), runs each instruction in order, and saves the result as a local image --- a self-contained snapshot that can run as a container on any machine with Docker installed. `docker push` uploads that image to a registry (`push: true`). Two smaller verbs round out the family: `docker images` lists the images on a machine, and `docker tag` gives an image a second name without rebuilding it. The `cache-` lines are why a rebuild that changes one late line of the Dockerfile takes minutes instead of an hour: every unchanged layer is reused.

Notice who runs this: a GitHub Actions machine, on every push to `main` and every `v*` tag (look back at the `on:` block). Nobody at PPBDS types `docker build`. That is how essentially every real image gets made --- by a runner, from a Dockerfile in a repo, on a trigger. If you ever want your Exercise 2 Dockerfile built and published for real, you would not run these commands either: you would ask an AI to add a workflow like this one to your repo, and the runner would do the rest.

### Exercise 4

Find the `tags: |` block inside the `Extract metadata` step. Copy and paste its four `type=` lines.

```{r the-devcontainer-image-4}
question_text(NULL,
	answer(NULL, correct = TRUE),
	allow_retry = TRUE,
	try_again_button = "Edit Answer",
...
These four patterns decide the **names** the image is published under. A release tagged `v1.1.5` produces `1.1.5` and `1.1`; every build of `main` also refreshes `latest`; and every build gets a `sha-` tag naming the exact commit. All of them live on GHCR, the same registry as any public image. This is the other half of a lesson from the previous tutorial: `codespace-starter` pins `ghcr.io/ppbds/devcontainer` to an exact `X.Y.Z`, never `latest`, because `latest` moves every time this workflow runs on `main` --- and a floating pin would change every student's Codespace without anyone deciding it should.

One more thing the workflow does that no student ever will: it logs in. `permissions: packages: write` plus `secrets.GITHUB_TOKEN` give the runner a temporary token that can push to GHCR, and the `Login to GHCR` step uses it. No personal token, no password, nothing to remember --- which is exactly why publishing an image is a job for a workflow rather than a person.
...
Find the seven `ARG` lines that pin tool versions (`QUARTO_VERSION`, `ARF_VERSION`, `NODE_MAJOR`, `CLAUDE_CODE_VERSION`, `CODEX_VERSION`, `GROK_VERSION`, `AIDER_VERSION`). Copy and paste them, and note which CLI mentioned nearby is the *exception* to this pinning approach.

```{r the-devcontainer-image-7}
question_text(NULL,
	answer(NULL, correct = TRUE),
	allow_retry = TRUE,
	try_again_button = "Edit Answer",
...
Pinning versions with `ARG` means every build installs the *exact same* Quarto, `arf`, Node, Claude Code, Codex, Grok, and aider --- until someone deliberately bumps a number. That's what makes the build reproducible. Note the comment above `AIDER_VERSION`: aider floated unpinned for a while, the one tool with a pin mechanism nobody was using, until v1.0.8 closed that gap. `agy` is the lone remaining exception: its installer has no version-pin option, so it floats with every rebuild. That's accepted as a known gap --- the CLI smoke test near the bottom still fails the build if a future `agy` release breaks something.
...
Find the `RUN` block that installs the four PPBDS course packages from GitHub. Copy and paste it, and note the two arguments to `pak::pkg_install()` that are not just the package list.

```{r the-devcontainer-image-12}
question_text(NULL,
	answer(NULL, correct = TRUE),
	allow_retry = TRUE,
	try_again_button = "Edit Answer",
...
The extra arguments are `upgrade = TRUE` and `dependencies = TRUE`.
...
`dependencies = TRUE` reaches further: it also installs each course package's **Suggests**. That is a deliberate contract --- a tutorial package's Suggests list *is* the set of packages students need at tutorial runtime, loaded with `library()` or named as tidymodels engine strings --- and "smoke test 1b" below re-reads those lists from the installed packages and loads every entry, so the contract is checked on every build.

Notice also the `echo` line and the `COURSE_PKG_REFRESH` date `ARG` just above the block. Docker caches layers by instruction text, so a rebuild with an unchanged Dockerfile would reuse this layer and silently ship stale course packages. Bumping the date changes the instruction text, which busts the cache for this layer and everything after it --- that is the whole job of that line.
...
This tutorial read through the real Dockerfile behind `ghcr.io/ppbds/devcontainer`, the image `codespace-starter`'s `devcontainer.json` points at. You saw why the base image is pinned by digest rather than a floating tag, why one image now serves both students and package developers, why layer order matters for Docker's build cache, and how deliberate `ARG` version pins keep builds reproducible. You also traced the permissions story that runs through the R package installs --- system libraries baked in as root, `PKG_SYSREQS=false` to stop `pak` from trying to `sudo`, and the switch to `USER rstudio` so lock files never poison the library for later installs --- and saw how the Python venv and the image-wide smoke tests each guard against a specific class of failure at build time, before a student ever opens a Codespace. Finally, you read the GitHub Actions workflow that builds and publishes the image --- `docker build` and `docker push` run by a machine on every push and every release, tagged by pattern, logged in with a token no human ever handles.

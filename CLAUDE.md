...
- **Later tutorials (and downstream packages):** just use the standard
  ...
  `whatever`. If you are not, create one and connect to it, as you
  learned in earlier tutorials.”* The mechanics were taught earlier, so
  this one sentence carries the whole setup — and its phrasing correctly
  covers the student who already connected before launching the
  tutorial. Downstream normal tutorials keep this environment-agnostic
  per the base guide — the student executes “create one and connect to
  it” the same way regardless of where they are.
